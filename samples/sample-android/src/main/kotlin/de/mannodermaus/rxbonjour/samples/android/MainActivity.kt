    ...
    nsdDisposable = rxBonjour.newDiscovery(type)
        ...
        .subscribe(
            { event ->
              ...
              when (event) {
                is BonjourEvent.Added -> if (!listAdapter.containsItem(item)) listAdapter.addItem(
                    item)
                is BonjourEvent.Removed -> if (listAdapter.containsItem(
                    item)) listAdapter.removeItem(item)
              }
            },
            { error ->
